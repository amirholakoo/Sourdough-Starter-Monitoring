...
void loop() {
  ...
  if (monitoring) {
    if (!sensorsError()) {
      ...
      // Handle error (e.g., enter a safe state, retry, or reset)
    }
    
    processSensors();
    delay(2000); // Reduce frequency of sensor readings
  }
}
...
bool sensorsError() {
  ...
  // Error checks for sensor values
  if (isnan(currentTemp) || isnan(currentHumid) || currentCO2 <= 0 || currentAlcohol <= 0) {
    return false; // Return false if any sensor reading is invalid
  }

  // ... [Rest of the function]

  return true; // Return true if all sensor readings are valid
}
